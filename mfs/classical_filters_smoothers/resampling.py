...
def _sorted_uniforms(n, key: JArray) -> JArray:
...
def _systematic_or_stratified(weights: JArray, key: JArray, is_systematic: bool) -> JArray:
    n = weights.shape[0]
    if is_systematic:
...
def systematic(weights: JArray, key: JArray) -> JArray:
    return _systematic_or_stratified(weights, key, True)


def stratified(weights: JArray, key: JArray) -> JArray:
    return _systematic_or_stratified(weights, key, False)


def multinomial(weights: JArray, key: JArray) -> JArray:
    n = weights.shape[0]
    idx = jnp.searchsorted(jnp.cumsum(weights),
...
def continuous_resampling(samples: JArray, weights: JArray, nsamples: int, key: JArray) -> JArray:
    ...
    Sheheryar Malik and Michael K. Pitt. Particle filters for continuous likelihood evaluation and maximisation.
